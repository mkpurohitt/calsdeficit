 ...
 *
 * The password-reset flow reuses this: it appends ?next=/auth/new-password so
 * that once the link has signed the user in, they land on a page where they can
...
export default function VerifyEmailLinkPage() {
  ...
  const completeSignIn = async (email: string) => {
    try {
      ...
      // Honour ?next= (the reset flow sends people on to set a password);
